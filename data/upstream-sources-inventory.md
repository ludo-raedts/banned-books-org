...
## Investigated — infeasible at title level (2026-06)

- **Ukraine — Derzhkomteleradio import-permit register** (comin.kmu.gov.ua). Law signed 30 Dec 2016, tightened to a near-total ban on Russian/Belarusian publications 19 Jun 2022. The committee denies import permits **case-by-case and publishes no public, per-title enumerated register**, so there is no scrapeable title-level source to adapt. Documented denials in press/Wikipedia are author/category-level (Dugin, Limonov, Glazyev; "two Akunin novels"; "bogatyr" children's books) and don't meet the title-specific bar. **Removed the "planned" entry from the public /sources page** (it was the only planned orphan and implied future work that isn't achievable). Capture notable individual UA cases from press/Wikipedia instead.

## Known but unprocessed (candidates for a future title-level pass)

- **GDR / East Germany (DD)** — the German Democratic Republic ran extensive literary censorship via the Hauptverwaltung Verlage und Buchhandel (HV; print-permit / "Druckgenehmigung" system). Scholarship (e.g. the "Zensur in der DDR" literature, the Stasi/HV archives at the Bundesarchiv) documents many specific denied or suppressed titles. We currently hold 8 DD records; a dedicated pass against the academic literature could yield a substantial title-level batch. Not yet attempted.

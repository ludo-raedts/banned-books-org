...
CREATE OR REPLACE VIEW v_weekly_totals AS
SELECT
...
CREATE OR REPLACE VIEW v_top_books_this_week AS
...
CREATE OR REPLACE VIEW v_top_books_last_week AS
SELECT entity_id, COUNT(DISTINCT visitor_hash)::bigint AS views
FROM pageviews
WHERE entity_type = 'book'
...
CREATE OR REPLACE VIEW v_top_authors_this_week AS
SELECT entity_id, COUNT(DISTINCT visitor_hash)::bigint AS views
FROM pageviews
...
CREATE OR REPLACE VIEW v_top_authors_last_week AS
SELECT entity_id, COUNT(DISTINCT visitor_hash)::bigint AS views
FROM pageviews
WHERE entity_type = 'author'
  AND viewed_at  >= now() - INTERVAL '14 days'
...
CREATE OR REPLACE VIEW v_top_countries_this_week AS
...
CREATE OR REPLACE VIEW v_top_countries_last_week AS
SELECT country, COUNT(DISTINCT visitor_hash)::bigint AS views
FROM pageviews
WHERE viewed_at >= now() - INTERVAL '14 days'
  AND viewed_at <  now() - INTERVAL '7 days'
...
CREATE OR REPLACE VIEW v_top_referrers_this_week AS
...
CREATE OR REPLACE VIEW v_top_referrers_last_week AS
SELECT referrer_host, COUNT(DISTINCT visitor_hash)::bigint AS views
FROM pageviews
WHERE viewed_at    >= now() - INTERVAL '14 days'
  AND viewed_at    <  now() - INTERVAL '7 days'

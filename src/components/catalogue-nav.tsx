        ...
        {items.map(({ Icon, title, text, href }) => {
          ...
          return (
            <Link
              key={title}
              href={href}
              className={`group flex flex-col bg-white border rounded-lg p-4 transition-colors ${
                isBBW
                  ...
                  ? 'border-brand/40 hover:border-brand/70 hover:bg-gray-50/50'
            ...
            >
              ...
              <span className="font-semibold text-gray-900 text-sm group-hover:text-brand transition-colors">{title}</span>
              <span className="text-xs text-gray-600 leading-snug mt-1">{text}</span>
              {isBBW && (

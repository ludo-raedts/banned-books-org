...
| `enrich-reasons.ts` | Vult ban-reasons voor rijen zonder bruikbare reason (GPT-4o-mini): alleen-'other' **én** (sinds 2026-09-03) rijen met géén enkele reason-link; rijen met een specifieke reason blijven ongemoeid. Kandidaat-ids worden in SQL bepaald (PostgREST kan geen NOT EXISTS), dus een run laadt alleen wat hij classificeert. Flags: `--class=untagged\|other\|both`, `--book-ids=` / `--ids-file=`, `--limit=N`, `--report=<pad>` (classificeert álle kandidaten en schrijft een review-artefact; dry-run print anders 5 samples) |

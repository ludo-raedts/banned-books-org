...
| `enrich-reasons.ts` | Vult ban-reasons voor rijen zonder bruikbare reason (GPT-4o-mini): alleen-'other' **én** (sinds 2026-09-03) rijen met géén enkele reason-link; rijen met een specifieke reason blijven ongemoeid. Vocabulaire komt uit de `reasons`-tabel (nooit hardcoded — 'blasphemy' stond wél in de prompt maar niet in de DB en verdween stil). Kandidaat-ids worden in SQL bepaald (PostgREST kan geen NOT EXISTS), dus een run laadt alleen wat hij classificeert. Flags: `--class=untagged\|other\|both`, `--book-ids=` / `--ids-file=`, `--limit=N`, `--report=<pad>` (classificeert álle kandidaten + JSON-tweeling), `--grounded-only` (schrijft alléén redenen die de ban-beschrijving zélf noemt, met lexicale verifier op de STATED-regel), `--apply-from=<json>` (schrijft de reviewde beslissingen verbatim, met exact-state guard) |
...
| `_fix_bbw_featured_data_2026_09_07.ts` | BBW-2026 pre-flight: #641 *The Ukrainian Night* verkeerde auteur (Mychailo Wynnycky → **Marci Shore**, Yale UP 2017) + jacket-pullquote uit de blurb; #1480 *Lexicon* droeg beschrijving én ISBN van *The Call of the Wild* (Lulu 9781329820418 → Penguin 9780143125426). Beide gestempeld `description_source_type='manual'` met bron-URL | one-off |
| `_seed_bbw_elsewhere_block_2026_09_07.ts` | Seedt + publiceert content-block `bbw-elsewhere`: de Nederlandse **Week van het Verboden Boek** (19–27 sep 2026, 2e editie; CPNB/Probiblio/PEN Nederland/Schrijverscentrale/Boekverkopersbond) op de BBW-hub. Vult de belofte in `bbw-what-is` ("other countries have adopted parallel initiatives") die nergens concreet werd. Rendert body_html via dezelfde `renderContentBlockHtml`-pijplijn als de admin-save. **Datums jaarlijks herzien** | one-off |

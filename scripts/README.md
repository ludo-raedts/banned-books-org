...
| `backfill-utah-statewide-bans.ts` | Utah USBE-lijst (HB 29 / §53G-10-103) | Titels die uit ÁLLE Utah public schools moeten; bans op bestaande boeken, `scope_id=4`, regio Utah. **Idempotent — bij een nieuwe USBE-lijst zet je de regel in `ROWS` en draai je 'm opnieuw**, niet een nieuw script. Sheet ophalen via `…/_layouts/15/download.aspx?share=<token>` (de gewone share-link geeft een HTML-shell). Uitgevoerd 2026-07-11 (36 titels), 2026-08-25 (37e: Push) | `--apply` |
| `_import_fsem_delta_2026_08_25.ts` | minjust FSEM entries #5500–5509 (inline, uit `/source-watch`) | One-off voor de maandelijkse FSEM-aanwas: 2 publicaties geïmporteerd (1 nieuw boek + 1 ban-only op bestaand werk #17105), 8 audio/muziek-entries buiten de books-only scope gelaten. Dunne lezer op de gedeelde commit-lib; sjabloon voor een volgende kleine FSEM-delta | `--apply` |
...
| `_audit_split_authors.ts` | **Staande** detector voor de "Lastname, Firstname."-catalogus-split (parser-bug `src/lib/wikipedia/parser.ts:567`, gefixt 2026-05-18). Draait live op de DB: single-token auteursnamen (optioneel met bron-punt) → clusters = boeken met ≥2 fragmenten; boeken met precies één fragment gaan in een zwakke tweede bucket (mononiemen als "Homer"/"Colette" wonen daar ook). Merk per rij: `SHARED` (book_count>1 → **unlink** van dit boek, nooit hernoemen) vs `EXCLUSIVE` (book_count=1 → veilig hernoemen). `--md` schrijft het review-document. Bestand was tot 2026-08-24 wel gecatalogiseerd maar nooit gecommit; `data/hk-split-authors-review.md` is de bevroren 2026-05 snapshot, `data/split-authors-review-<datum>.md` de actuele run |
...
| Script | Doet |
|---|---|
| `enrich-all.ts` | **Master-pipeline.** Fase 1 = parallelle gratis-harvest (ol/gb/native-titles concurrent — verving de oude sequentiële `enrich-isbn` + covers-continuous first-pass), Fase 2 = sequentieel covers-v2 + gutenberg + archive + descriptions-v2 + GPT. Wrapt alles in een before/after coverage-snapshot + confidence-rollback. Flags: `--apply`, `--free-only`, `--no-gutenberg`, `--no-archive`, `--gpt-limit=N`, `--native-limit=N`, `--threshold=`, `--heartbeat=N` (statusregel elke N s in het hoofdlog — Fase 1 toont de laatste logregel per nog-lopende harvester, Fase 2 een levensteken met verstreken tijd; default 60) |
| `enrich-status.sh` | **Read-only status-check** van een lopende (of laatste) `enrich-all`-run vanuit een tweede terminal: draait-hij-nog + per-harvester laatste logregel + tail van `data/enrich-all.log`. `-f [sec]` = live verversen |
...
| `enrich-descriptions-websearch.ts` | **Web-gesourcete descriptions** voor rijen waar de v2-ladder leeg op liep. `--mode=altisbn`: GB/OL **per ISBN** (incl. het nooit eerder geprobeerde `bookshop_isbn13`); guard-rejects verklikken foute Bookshop-deeplinks. `--mode=search`: Gemini-grounded site-zoektocht (hergebruikt `gemini-cover-search`) → `extract-description-from-page` (JSON-LD/og/meta; Firecrawl-fallback bij botwalls — bookshop.org & goodreads 403'en plain fetches). Alles door gpt-4o-mini judge+groom (nooit letterlijke marketing-copy); schrijft `llm_grounded_single` + canonieke bron-URL, status `default`. Flags: `--mode=`, `--lang=`, `--limit=`, `--slugs=`, `--apply`. Rapport + JSONL-checkpoint in `data/` = resumebaar. **Altijd eerst dry-run + report-review** |
| **Covers** | |
...
| `enrich-kobo-links-site.ts` | **Tier 2** na `enrich-kobo-links.ts`: lost de sticky-missers op via Kobo-site-zoek (Firecrawl-scrape; Rakuten-feed mist agency-priced Big-5 ebooks = juist de beroemde titels). Zelfde guards (`scripts/lib/kobo-match.ts`), ban-count-volgorde, 1 scrape/boek — draai voor de top, niet de hele tail | apply |
| `enrich-kobo-links.ts` | Vul `books.kobo_url` (kaal Kobo-product-URL, affiliate-wrap gebeurt render-side) via Rakuten Product Search API (mid=37589); keyword = hoofdtitel + auteur-achternaam, title-token-guard ≥60%; sticky via `kobo_checked_at` + `--stale-before`. Vereist `RAKUTEN_CLIENT_ID/SECRET/SID` in .env.local | apply |
...
| `enrich-genres-gpt.ts` | GPT genre-enrichment (lege `genres`), 1–3 slugs uit de 25-slug vocabulary. Grounding-guard: rijen zónder `description_book` worden alleen geaccepteerd als GPT én Gemini onafhankelijk dezelfde slug geven. **Requeue-route** (2026-09-03): `--ids=` / `--ids-file=` her-beoordeelt een expliciete id-lijst en impliceert `--overwrite`, zodat rijen die al een genre dragen gecorrigeerd kunnen worden — dit is hoe de `literary-fiction`-stempel van de oude `import-pen.ts`-regex wordt teruggedraaid (worklist uit `_audit_literary_fiction_default.ts --write-worklist`). Een re-grade die leeg/low-confidence terugkomt laat de bestaande waarde staan, hij blankt de rij niet |
| `enrich-genres-retry-gpt.ts` | Retry-pass voor wat gpt-4o-mini niet plaatste (zelfde logica, sterker default-model; erft `--ids`/`--ids-file`) |
| `enrich-reasons.ts` | Vult ban-reasons voor rijen zonder bruikbare reason (GPT-4o-mini): alleen-'other' **én** (sinds 2026-09-03) rijen met géén enkele reason-link; rijen met een specifieke reason blijven ongemoeid. Scope met `--book-ids=` / `--ids-file=`, anders laadt hij de hele bans-tabel |
...
| **Awards** | |
| `enrich-awards.ts` | Vult de `awards` JSONB op authors (Nobel, author-level via api.nobelprize.org CC0) en books (Pulitzer Fiction/Novel, work-level via Wikidata SPARQL Q833633/Q1155483). Re-fetcht beide bronnen elke run; idempotent (zelfde `(award,category)`-key wordt vervangen, rest blijft). Hand-geverifieerde excludes ingebakken (Faulkner-"A Fable"-collision id 1286, Maus special citation). Matching = genormaliseerd titel+auteur (geen ISBN op Wikidata). Dry-run default; `--apply`. Zie `data/award-overlap.md` + migratie 20260613100000. Re-run wanneer nieuwe award-winnende boeken zijn toegevoegd |
...
| **Genres** | |
| `_audit_genre_vocabulary.ts` | **Canonieke genre-detector** (catalogus-breed, read-only). Toetst `books.genres` tegen de 25-slug vocabulary in `src/components/genre-badge.tsx` (`isMappedGenre()`): (1) elke slug buiten de map met count + wat `scripts/lib/genre-folds.ts` ervan zou maken, (2) rijen boven de 1–3-slug cap, (3) in-vocabulary slugs op het verkeerde soort boek (`non-fiction` + `political-fiction`), (4) de `literary-fiction`-concentratie. Nulmeting 2026-09-03: 48 slugs in de DB tegen 21 in de map = 411 strays op 403 boeken; ná de sweep 25 slugs / 0 strays. De off-vocabulary-count is nu ook een harde invariant in `audit-integrity.ts`. `--verbose` |
| `_audit_literary_fiction_default.ts` | **Root-cause audit** van `literary-fiction` op 40% van de catalogus. Draait de verwijderde `guessGenres()`-ladder uit `import-pen.ts` opnieuw over live data — 93,7% van de PEN-boeken droeg exact wat die ladder uitspuugt, 6.472 via de onvoorwaardelijke fall-through — zodat de claim controleerbaar blijft. Toont ook de branches die actief mislabelen ("The Rape of Nanking" → historical-**fiction**, "Memoirs of a Geisha" → memoir) en waarom de stempel erger is dan een lege waarde: `enrich-genres-gpt.ts` target `genres = '{}'`, dus de stempel sloot de rij permanent uit van de classifier. `--write-worklist` schrijft `data/genre-requeue-ids.txt` voor de re-grade |
| `_audit_graphic_novel_mistags.ts` | Strips comics die als proza staan geboekt — de hoogste-opbrengst-deelverzameling van het `literary-fiction`-probleem (PEN 2021+ is comics-zwaar). Drie tiers: **STRONG** (de titel zégt het: "The Graphic Novel", "A Graphic Memoir", "Manga Shakespeare"), **PROBABLE** (de description noemt de vorm), **WEAK** (alleen illustrator-credit/panels — niet bulk-actionable). Negatieve guards weren proza *óver* comics ("Why Comics?", "The Art of Comics", "A Brief History of Manga", de "Manga Dinosaurs" how-to-draw-serie), romans met comics in de plot ("Verona Comics") en geïllustreerde kinderboeken die geen sequential art zijn. `--verbose` |
| **Jaren** | |
...
| `cleanup-vague-pen-rollups.ts` | Verwijdert vage PEN-aggregaat roll-ups (institution+region NULL, enige bron = generieke `pen.org/book-bans/`) die gedekt zijn door een concrete districtrij zelfde boek/land — **jaar telt niet mee** (revisie 2026-09-01: het roll-up-jaar is zelf niet gesourced, de seed-import vulde `first_published_year + 1`, dus het oude jaar±1-venster hield 265 onverantwoorde rijen vast); keepers zonder concrete dekking (30) en rijen met extra bronnen (14) blijven staan, laatste geflagd voor handmatige review; JSON-backup per run; idempotent — herdraai na elke aggregaat-achtige PEN-import | generiek |
...
| `_fix_genre_vocabulary.ts` | Apply-zijde van `_audit_genre_vocabulary`: past `scripts/lib/genre-folds.ts` toe — foldt off-vocabulary slugs op hun canonieke equivalent (`young-adult-fiction` → `young-adult`, `essays` → `essay`, `nonfiction` → `non-fiction`, …), dropt wat nooit een genre was (kale `fiction`, `war`, `lgbtq`, `novella`), repareert `non-fiction` + `political-fiction` → `political-non-fiction`, en trimt boven de 3-slug cap. Een rij die leeg overblijft wordt bewust `'{}'` — dat is de eerlijke staat voor een ongeclassificeerd boek en zet hem terug in de `enrich-genres-gpt.ts`-pool. Schrijft alleen gewijzigde rijen, gebatcht per doelwaarde (462 rijen in 79 statements); weigert te schrijven als de fold-tabel een slug buiten de map zou opleveren. Idempotent | generiek |
| `_fix_graphic_novel_mistags.ts` | Apply-zijde van `_audit_graphic_novel_mistags`, **alleen de hand-geverifieerde STRONG-tier** (28 rijen): boeken waarvan de titel de vorm declareert maar die een proza-genre droegen — Manga Shakespeare, de Anne Frank graphic adaptation/biography, *The Gettysburg Address: A Graphic Adaptation*, Bechdel's *Are You My Mother?*, en #502 *Maus 1* (stond op `historical-fiction`, terwijl zusterrij #122 *Maus* al correct was). Guards: elke slug tegen `isMappedGenre()`, max 3, `graphic-novel` moet eerst staan (vorm leidt), en een titel-drift-check per id. De 300+ PROBABLE-rijen gaan bewust via de classifier, niet via deze tabel | one-off |
| `_fix_hinds_graphic_novel_genres.ts` | De point-fix die deze hele genre-audit aanzwengelde: de vier Gareth Hinds-adaptaties (#2110 Beowulf, #7547 The Odyssey, #7598 The Iliad, #18043 Macbeth). Twee stonden op `literary-fiction` — de import-default — wat het enige feit dat telt verbergt: het zijn strips. Gevonden 2026-09-03 bij de Tennessee-"Odyssey"-bans (Wilson County en Oak Ridge verbanden de Hinds-adaptatie, niet Homerus) | one-off |
...
| `remediate-bookshop-editions.ts` | Fixt geflagde link-ISBNs: zoekt Engelse papieren editie op het OL-werk, probet op Bookshop, anders demote naar storefront; `--force-english` voor vertaalde klassiekers |

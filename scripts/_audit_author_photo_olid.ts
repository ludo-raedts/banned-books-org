 ...
 *
 * Method — normalised surname-token match, following the token-match doctrine
 * of src/lib/enrich/title-match.ts (accent-stripped, lowercased, alnum-split,
 * single-letter initials dropped) with three additions the author case needs:
 *   • inverted forms — OL stores plenty of "Surname, First" records, so the
 *     surname is taken from before the comma when there is one;
 *   • name particles (van, de, von, bin, …) never count as shared evidence on
 *     their own;
 *   • pen names / transliterations — `alternate_names` and `personal_name` are
 *     BOTH accepted as a match, so "Dr. Seuss" ↔ "Geisel, Theodor Seuss" and
 *     Cyrillic/CJK alternates clear instead of flagging.
 * Deliberately asymmetric: a match on ANY OL name variant clears the row. We
 * are hunting confident contamination, not scoring name similarity.
 *
 ...
 */
type Script = 'latin' | 'cyrillic' | 'greek' | 'han' | 'kana' | 'hangul' | 'arabic' | 'hebrew' | 'other'
...
/**
 * Compare our display_name against every OL name variant. A surname agreement
 * on ANY variant clears the row — pen names, maiden names and transliterated
 * alternates are all legitimate ways for the same person to be recorded.
 */
...
function writeReport(probes: Probe[]) {
  ...
  lines.push('')
  lines.push(
    '`tryOpenLibrary()` in `src/lib/enrich/author-photos.ts` queries ' +
      '`/search/authors.json?q=<our name>` and accepts the first of up to 3 docs that has ' +
      '`work_count >= 1` and a HEAD-able photo — **it never compares `doc.name` to ours**. ' +
      'So any author whose name merely *retrieves* a photo-bearing OL record inherits that ' +
      "record's face: \"Troy Andrews\" pulled Lauran Paine, whose `alternate_names` include " +
      '"Troy Howard (pseud.)". Nulling a row here is therefore only half a fix — the default ' +
      'enricher gate (`photo_url IS NULL AND photo_v2_checked_at IS NULL`) will skip it, but ' +
      '`--recheck` would re-pin the same wrong photo until that branch gets a name gate ' +
      '(`matchNames()` in this script is the check it lacks).',
  )
  lines.push('')

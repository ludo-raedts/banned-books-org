...
- Do NOT propose duplicates of events already in the database. Treat an existing row as the SAME event when it has the same country and a year within ±1 (school-year boundaries make off-by-one years common), EVEN IF its scope_id or action_type differs from how you would code it — sources often code one real-world event differently. Instead, propose an "update" if AND ONLY IF the article adds NEW concrete detail not yet present in the existing row.
- NEVER target an update at an existing row whose institution is set unless the article explicitly mentions that institution. Per-district rows (e.g. institution="Broward County Public Schools") describe ONE local event; do not patch them with state-level or country-level facts, and do not attach generic descriptions to them. If the article only supports a broader event, propose a new ban with the appropriate region/institution instead.

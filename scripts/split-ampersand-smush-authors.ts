/**
 * Splits author-rijen die meerdere personen smushen in één display_name
 * ("Karl Marx & Friedrich Engels", "Michael & Dorothy Clarke", "Lofu and
 * Others") in losse author-rijen. Re-implementeert de classificatie uit
 * scripts/_audit_ampersand_smush_authors.ts.
 *
 * Acties per categorie:
 *
 *   SPLIT / SPLIT_SURNAME
 *     - Voor elke parsed naam: find-or-create canonical author (slug-match
 *       eerst, dan exacte display_name-match, anders INSERT nieuwe rij).
 *     - book_authors-link verhuist van smush-rij naar elk van de losse
 *       authors (rol komt uit smush-rij; bij "Translated By"-prefix wordt
 *       het 'translator').
 *     - Smush-rij wordt verwijderd.
 *
 *   STRIP_OTHERS
 *     - Rename naar de eerste naam zonder "and Others"-staart. Als er al
 *       een canonical author met die naam bestaat, MERGE in plaats van
 *       rename: link verhuist + smush-rij weg.
 *
 *   SKIP_PEN_NAME / SKIP_ORG / SKIP_ANON
 *     - Geen actie.
 *
 * Outputs:
 *   - data/split-ampersand-newly-created-ids.json   — IDs van nieuwe authors
 *     (voor de daaropvolgende enrichment-stap)
 *
 * Usage:
 *   pnpm tsx --env-file=.env.local scripts/split-ampersand-smush-authors.ts          # dry-run
 *   pnpm tsx --env-file=.env.local scripts/split-ampersand-smush-authors.ts --apply  # mutate
 */
...
async function insertAuthor(sb: ReturnType<typeof adminClient>, displayName: string): Promise<Author> {
  ...
  // Slug-collision: vrijwel altijd al gevangen door findExisting, maar voor
  // robuustheid: hang -2 / -3 / … achter de slug
...
async function main() {
  ...
  console.log(`── split-ampersand-smush-authors ── (${APPLY ? 'APPLY' : 'DRY-RUN'})\n`)

  // Verzamel kandidaten
  const { data: amp } = await sb.from('authors').select('id, display_name, slug').ilike('display_name', '%&%').order('id')
  const { data: andd } = await sb.from('authors').select('id, display_name, slug').ilike('display_name', '% and %').order('id')
  const seen = new Set<number>()
  const rows: Author[] = []
  for (const r of [...(amp ?? []), ...(andd ?? [])] as Author[]) {
  ...
  for (const row of rows) {
    ...
    if (action.kind === 'STRIP_OTHERS') {
      ...
      // Bestaat de canonieke al?
      let target = await findExisting(sb, newName)
      try {
        if (target && target.id === row.id) {
          // Zelfde rij: alleen display_name + slug bijwerken
    ...
    try {
      ...
      // Voor elk boek-link: voeg link toe naar elk target
      for (const l of bookLinks) {
  ...
  if (APPLY) {
    ...
    // Schrijf newly-created IDs naar disk voor enrich-stap
    const outPath = join(process.cwd(), 'data', 'split-ampersand-newly-created-ids.json')
